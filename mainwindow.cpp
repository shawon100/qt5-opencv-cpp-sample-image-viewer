...
void MainWindow::createActions()
{
    ...
    bw = new QAction("&Black&White", this);
    viewMenu->addAction(bw);
    ...
    connect(bw, SIGNAL(triggered(bool)), this, SLOT(blackwhite()));
    setupShortcuts();
}
...
void MainWindow::openImage()
{
    ...
    if (dialog.exec()) {
        ...
        image=filePaths.at(0);
        //cvimage=cv::imread(image.toStdString());
        //cv::cvtColor(cvimage, image_gray, cv::COLOR_BGR2GRAY);
        //cv::imwrite(image.toStdString(),image_gray);
        showImage(image);
    }
}
...
void MainWindow::blackwhite()
{
    ...
    extension=QString::fromStdString(extensionstr);
    num= QString::number(i);
    backup_image=name+num+extension;
    i=i+1;
    cv::imwrite(backup_image.toStdString(),cvimage);
    showImage(image);
